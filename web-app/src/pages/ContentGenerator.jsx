...
const LS_DRAFT_PROMPT  = 'contentGenerator_draft_prompt';
const LS_DRAFT_TONE_ID = 'contentGenerator_selectedToneId';
const LS_DRAFT_BASE_ID = 'contentGenerator_selectedBaseId';
const SS_DRAFT_IMAGES  = 'contentGenerator_draft_images';
...
// FastAPI returns 422 as { detail: [{loc, msg, type}, ...] } and
// HTTPException returns { detail: "string" }. Rendering the array or
// object shapes directly in a toast would crash React with error #31
...
const ContentGenerator = () => {
  ...
  const handleKBUploadFile = async (e) => {
    ...
    try {
      ...
      // 1. Create an empty KB first. Name defaults to the file name without
      //    extension so users can rename later via the edit flow.
      ...
      // Best-effort rollback: if we managed to create the KB but failed to
      // upload, drop the empty row so the user doesn't see a "Pendente" ghost.
      if (createdKbId) {
        try {
          const token = await getAccessToken();
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const msg = formatApiError(err, 'Erro ao gerar conteúdo. Tente novamente.');
      setErrorToast(msg);
  ...
  // ESC: close adjust popup first, then developed modal, then clear selections
  useEffect(() => {
    const h = (e) => {
      if (e.key !== 'Escape') return;
      if (adjustingIdea) {
        setAdjustingIdea(null);
      } else if (developedViewing) {
    ...
    document.addEventListener('keydown', h);
    return () => document.removeEventListener('keydown', h);
  }, [adjustingIdea, developedViewing, selectedIdeas, selectedDeveloped]);
  ...
  return (
    <div className="relative flex flex-col h-screen transition-all duration-300" style={{ marginLeft: collapsed ? 72 : 260 }}>
            ...
            >
              ...
              <div className="pt-6 px-7 pb-5 border-b border-white/[0.06] flex items-start justify-between gap-4 shrink-0">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    onClick={() => setAdjustingIdea(developedViewing)}
      ...
      {/* ═══ SEND DESTINATION MODAL (Kanban / Calendar) ═══ */}
      <AnimatePresence>
        {sendDest && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            ...
            >
              ...
              <div className="px-6 pt-5 pb-4 border-b border-white/[0.06] flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <h3 className="text-[15px] font-extrabold text-white tracking-tight">
                    {sendDest.mode === 'kanban' ? 'Enviar para o Kanban' : 'Enviar para o Calendário'}
                  </h3>
                </div>
                <button
                  onClick={closeSendDest}
              ...
              <div className="px-6 py-5 space-y-4">
                ...
                {projects.length === 0 ? (
                  <div className="p-4 rounded-xl bg-amber-500/[0.06] border border-amber-500/20 text-amber-400 text-[12px] font-medium">
                    Você ainda não tem projetos. Crie um projeto no Kanban antes de enviar conteúdos.
                  </div>
                ) : (
                  <>
                    {/* Project selector */}
                    ...
                    {sendDest.mode === 'kanban' ? (
                      ...
                      <div>
                        <label className="block text-[11px] font-bold uppercase tracking-widest text-white/50 mb-2">Data</label>
                        <input
                          type="date"
                          value={sendDate}
                          onChange={(e) => setSendDate(e.target.value)}
                          className="w-full bg-[#0e0e11] border border-white/[0.08] rounded-xl px-3.5 py-2.5 text-[13px] text-white outline-none focus:border-primary/40 transition-colors [color-scheme:dark]"
                        />
                      </div>
                    )}
                  </>
              ...
              <div className="px-6 py-4 border-t border-white/[0.06] flex items-center justify-end gap-2.5 bg-black/20">
                ...
                <button
                  onClick={confirmSend}
                  disabled={sending || projects.length === 0 || !sendProjectId || (sendDest.mode === 'calendar' && !sendDate)}
                  className="flex items-center gap-2 px-5 py-2 rounded-lg bg-primary hover:bg-primary/90 text-white text-[12px] font-bold transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                >
